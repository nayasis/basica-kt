...
/**
 * ODS exporter for multiple sheets
 */
class OdsMultiExporter(
    sheets: Map<String, DataFrame>,
    private val showLabel: Boolean = true,
    private val startIndex: Int? = null,
) : DataFrameExporter() {

    private val sheetDefs = sheets.entries.map { entry ->
        SheetDef(
            name = entry.key,
            dataframe = entry.value,
            first = startIndex?.takeIf { it >= 0 } ?: 0,
    ...
    private fun writeContent(zos: ZipOutputStream) {
        ...
        sheetDefs.forEach { sheet ->
            val table = spread.appendElement("table:table").apply {
                setAttribute("table:name", sheet.name)
            }
            val header = table.appendElement("table:table-row")
            for (label in if (showLabel) sheet.dataframe.labels else sheet.dataframe.keys) {
                header.appendCell(label)
            }
            var emptyRowCount = 0
            for (i in sheet.first..sheet.last) {
                val isEmptyRow = sheet.dataframe.isRowEmpty(i)
                if (isEmptyRow) {
                    emptyRowCount++
                } else {
                    if (emptyRowCount > 0) {
                        table.appendElement("table:table-row").apply {
                            setAttribute("table:number-rows-repeated", emptyRowCount.toString())
                            appendElement("table:table-cell").apply {
                                setAttribute("table:number-columns-repeated", sheet.dataframe.keys.size.toString())
                            }
                        }
                        emptyRowCount = 0
                    }
                    val row = table.appendElement("table:table-row")
                    for (key in sheet.dataframe.keys) {
                        row.appendCell(sheet.dataframe.getData(i, key))
                    }
                }
            }
            if (emptyRowCount > 0) {
            ...
            when {
                ...
                isDateObject(value) -> {
                    ...
                    if (odsDate != null) {
                        ...
                        when (value) {
                            is LocalDateTime, is Date, is ZonedDateTime -> setAttribute("table:style-name", "cellstyleDatetime")
